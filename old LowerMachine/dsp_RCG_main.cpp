...
//关于使用静态函数的设计：
//对象和对象之间的成员变量是相互独立的 要想共享数据 则需要使用静态成员;
//静态函数只能在当前文件下使用
//常见的使用方法:
//static int fun() {} 当我们希望在多个类中调用fun函数时 必须把fun函数声明为static 否则在编译器编译时辉产生多个关于fun的定义
//尤其当我们把在C环境中写好的程序移植到C++中去时需要加上static 
...
//这里是旧版本关于步态划分的设计 新版本中实现了六个动作以及每个动作的五个步态相位的划分;
void dsp_RCG_main()
{
	init_sig();		//��ʼ���ź�
	init_glbs();	//��ʼ��ȫ��
