...
dsp_RCGClass::dsp_RCGClass()
{
	...
	//走 跳 上楼梯 下楼梯 上坡 下坡
	const char* modelpath[7] = {"./model/actions.tflite","./model/uphill.tflite","./model/downhill.tflite","./model/upstairs.tflite","./model/downstairs.tflite","./model/walk.tflite","./model/jump.tflite"};
	for(int i=0;i<7;++i){
		//加载模型
...
bool dsp_RCGClass::Recg()
{
    ...
    float* input_IMU = interpreter[0]->typed_tensor<float>(inputs_indexarray[0]);
	int inputemgindex = 0;
	int inputimuindex = 0;
    ...
    float* gaitinput_IMU = interpreter[U+1]->typed_tensor<float>(gaitinputs_indexarray[0]);
	...
	V = softmax(gaitmodeloutput, 5);
	#else
	std::vector<int> gaitinputs_indexarray = interpreter[U+1]->inputs();  //输入张量的索引数组
	float* gaitinput_EMG = interpreter[U+1]->typed_tensor<float>(gaitinputs_indexarray[0]);//填充数据即可
    float* gaitinput_IMU = interpreter[U+1]->typed_tensor<float>(gaitinputs_indexarray[1]);
	int gaitinputemgindex = 0;
	int gaitinputimuindex = 0;
	for(int row=0;row<20;++row){
		for(int col = 0;col<10;++col){
	...
	for(int i=0;i<5;++i){
		outputsANS[i] = gaitmodeloutput[i];
	}
	...
	for(int i = 0;i<6;i++)
	{
		BP_off<<modeloutput[i]<<',';
	}

	BP_off<<gaitV<<',';
	for(int i = 0;i<5;i++)
	{
		BP_off<<gaitmodeloutput[i]<<',';
	}
